...
def process_item_page(db, page_source,link_adress):
    ...
    handle_page(None,handle_overview,handle_brief,None,soup,db,link_adress)
...
def handle_page(pic_sub,overview_sub,brief_sub,thing_sub,soup,db,link):
   #global soup
...
'gender', 'fee']
    ...
    item.address,item.update_time, item.due,link, item.gender,item.fee,item.phone]],   \
    columns=['house_type','room_type','price','area','floor',
    'address','update_time','due','link' ,'gender', 'fee','phone'])
    print new_item
...
def handle_overview(section,item):
    ...
    print_elements(lis,False)
    lis_dic={}
    (area,floor,houst_type,room_type,community)=('','','','','')
    for each in lis:
...
def trace_test(browser,test_pages,times=-1):
    ...
    for each_link in test_pages:
        ...
        time.sleep(1)
        process_item_page(db_item,browser.page_source,each_link)
        if times ==0 : break
    db_item.to_excel()
    pass
...
if __name__ == "__main__":
    ...
    test_pages=[' https://rent.591.com.tw/rent-detail-5680787.html ']
    time.sleep(1)
